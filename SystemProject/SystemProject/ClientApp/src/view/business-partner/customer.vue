  ...
  <div id="app">
   <v-app id="inspire">
     <div class="pb-1" role="alert">
      ...
      <v-card>
        <v-data-table fixed-header height="450" :headers="datas" :items="objCustomer" :search="search" dense>
           <template>
              <v-toolbar  flat>
     ...
     data () {
      return {  
        ...
        mounted(){         
          ...
          const _this = this;                  
          axios.get('/api/customer/GetCustomer',{headers: {Authorization : "Bearer "+localStorage.getItem('token')}}).then(response => {           
            _this.objCustomer = response.data;
        })
        },
        methods: {
          editItem (item) {
            localStorage.setItem('customerId', item);
