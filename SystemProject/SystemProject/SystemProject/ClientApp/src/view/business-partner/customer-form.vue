    ...
    <form class="bg-white p-5 rounded-lg shadow-lg min-w-full">
      <div class="flex flex-wrap -mx-2 space-y-4 md:space-y-0">
        <div class="w-full px-2 md:w-1/2">
          ...
          <input id="border-username" class="w-full h-8 px-2 text-sm rounded-lg focus:outline-none border-solid border" v-model="customer.code" type="text" :placeholder="$t('Code')" autocomplete="off"/>
        </div>
        <div class="w-full px-2 md:w-1/2">
          <label class="text-gray-800 font-semibold block my-3 text-md">{{ $t('Phone') }}</label>
          <input id="border-username" class="w-full h-8 px-2 text-sm rounded-lg focus:outline-none border-solid border" v-model="customer.phone" type="text" :placeholder="$t('Phone')" autocomplete="off"/>
        </div>
        <div class="w-full px-2 md:w-1/2">
          <label class="text-gray-800 font-semibold block my-3 text-md">{{ $t('Name 1') }}</label>
          <input id="border-username" class="w-full h-8 px-2 text-sm rounded-lg focus:outline-none border-solid border" v-model="customer.name1" type="text" :placeholder="$t('Name 1')" autocomplete="off"/>
        </div>
        <div class="w-full px-2 md:w-1/2">
          <label class="text-gray-800 font-semibold block my-3 text-md">{{$t('Address')}}</label>
          <input id="border-confirm" class="w-full h-8 px-2 text-sm rounded-lg focus:outline-none border-solid border" v-model="customer.address" type="text" :placeholder="$t('Address')" autocomplete="off" />
        </div>
      </div>
      <div class="flex flex-wrap -mx-2 space-y-4 md:space-y-0">
        <div class="w-full px-2 md:w-1/2">
          <label class="text-gray-800 font-semibold block my-3 text-md">{{$t('Name 2')}}</label>
          <input id="border-password" class="w-full h-8 px-2 text-sm rounded-lg focus:outline-none border-solid border" v-model="customer.name2" type="text" :placeholder="$t('Name 2')" autocomplete="off" />
        </div>
        <div class="w-full px-2 md:w-1/2">
          <label class="text-gray-800 font-semibold block my-3 text-md">{{$t('Location')}}</label>
          <input id="border-confirm" class="w-full h-8 px-2 text-sm rounded-lg focus:outline-none border-solid border" v-model="customer.location" type="text" :placeholder="$t('Location')"  autocomplete="off"/>
        </div>
      </div>      
      <div class="flex flex-wrap -mx-2 space-y-4 md:space-y-0">
...
<script>
  ...
  export default {
    ...
    methods: {
      saveData() {        
        let _this = this;        
        axios.post('/api/customer/CreateCustomer/customer', this.customer).then((response) => {          
      ...
      clearData() { 
        ...
        axios.get('/api/customer/GetCustomerEdit',{params:{customerId: customerId}}).then(response => {           
    ...
    mounted() {
      ...
      var customerId = localStorage.getItem('customerId');   
      axios
      .get('/api/customer/GetCustomerEdit',{params:{customerId: customerId}}).then(response => {           
          _this.customer = response.data;            
      })
    },
  };
